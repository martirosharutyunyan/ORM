...
exports.client = new pg_1.Client({
    ...
    password: process.env.POSTGRESDBNAME,
    port: 5432,
    host: 'localhost',
    user: 'postgres',
});
    ...
    constructor(TABLENAME, columns, params = { force: false, logging: false }) {
        ...
        try {
            ...
            if (params.force) {
                ...
                let str = '';
                this.columns.forEach((elem, i, array) => {
                    const column = `${elem.columnName} ${elem.type} NOT NULL${i === array.length - 1 ? '' : ','}`;
                    str += `${column}`;
                });
                const SQLQuery = `CREATE TABLE ${TABLENAME} (id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, ${str});`;
...
ORM.FLOAT = 'FLOAT';
ORM.BOOLEAN = 'BOOLEAN';
exports.Table = new ORM('users', {
    column1: ORM.STRING,
    column2: ORM.INTEGER,
}, { force: true, logging: true });
// Table.findAll({where:{column1:"ok"}}).then(res => console.log(res))
